...
int main(int argc, char* argv[]) {
    ...
    std::vector<uint64_t> plain = {47483647, 10000};
    ...
    std::vector<uint64_t> remained_shares;
    cryptoUtil.decryptCipherEntryToInt64Vec(ce, remained_shares);
    // printf("%lld, %lld\n", remained_shares[0], remained_shares[1]);

    printf("%lld, %lld\n", (uint64_t)(shares[0] + remained_shares[0]), (uint64_t)(shares[1] + remained_shares[1]));
    ...
    // std::vector<uint64_t> merged_plain;
