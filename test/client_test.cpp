...
int main(int argc, char* argv[]) {
    ...
    client_oblivious_mapper_online(srcPos, dstPos, srcSvv, dstSvv, plainNumPerPos, 0, 0, 1);
    ...
    std::vector<std::array<uint64_t, 2>> result;
    uint64_t printNum = 10;
    for (int i=0; i<dstPosNum; ++i) {
        result.push_back({{dstSvv[i][0], dstSvv[i][1]}});
        if (i < printNum) std::cout<<dstPos[i]<<" "<<dstSvv[i][0]<<" "<<dstSvv[i][1]<<std::endl;
    }

    // Result validation
    osuCrypto::Channel& chl = *clientTaskComm.getChannel(1-tileIndex);
    chl.asyncSendCopy(dstPos);
    chl.asyncSendCopy(result);
