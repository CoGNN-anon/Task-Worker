...
#ifdef USE_FHE
#define MAX_SEND_CEV_SIZE 100
#else
#define MAX_SEND_CEV_SIZE 10000
#endif

// Fixme: The global variable is defined for linking SCI
int party = 0;
int num_threads = 4;
int bitlength = 32;
int port = 8000;
...
void task_queue_handler(std::queue<Task>* taskq) {
    ...
    for (int i=0; i<taskvs.size(); i++) {
        ...
        if (i != tileIndex) {
            ...
            uint64_t cur_taskv_size = taskvs[i].size();
    ...
    for (int i=0; i<pceVecs.size(); i++) {
        if (i != tileIndex) {
            uint64_t cur_pceVec_size = pceVecs[i].size();
...
void single_destination_task_vector_handler(std::vector<Task>& taskv, size_t dstTid) {
    ...
    if (thc.sendOperand) {
        ...
        int plainNumPerCE = 0;
        uint64_t cur_taskv_size = taskvs[dstTid].size();
        for (int j= 0 ; j<cur_taskv_size; j++) {
            if (j % 100000 == 0) printf("Prepare pack share j = %d\n", j);
        ...
        // Split random share & Send packed ciphertext share decrypting
        uint64_t cur_pceVec_size = pceVec.size();
...
uint64_t client_task_vector_direct_handler(std::vector<Task>& taskv, size_t dstTid) {
    ...
    if (!taskComm.getIsNoDummyEdge()) { // round-halved
        if (taskv.size() !=0) {
            if (thc.rotation > 1) {
...
void set_up_mpc_channel(bool isClient, uint32_t dstTid) {
    if (isClient) sci::setUpSCIChannel(emp::ALICE, dstTid);
    else sci::setUpSCIChannel(emp::BOB, dstTid);
}

void close_mpc_channel(bool isClient, uint32_t dstTid) {
    if (isClient) sci::closeSCIChannel(emp::ALICE, dstTid);
    else sci::closeSCIChannel(emp::BOB, dstTid);
}
...
void TaskComm::setUp(bool isClient) {
    ...
    ot_senders.resize(tileNum);
    ot_recvers.resize(tileNum);
    for (int i = 0; i < tileNum; ++i) {
        ot_senders[i] = new(osuCrypto::SilentOtExtSender);
        ot_recvers[i] = new(osuCrypto::SilentOtExtReceiver);
    }
    ...
    ioService = new osuCrypto::IOService(0);
    ...
    for (uint32_t j = 0; j < tileNum; ++j) {
        if (j != tileIndex) {
            ...
            if (!isCluster_ || isHost) {
                endpoints.emplace_back(*ioService, ip, port, isHost?osuCrypto::SessionMode::Server:osuCrypto::SessionMode::Client, name);
            } else {
                std::string remoteIp = "10.0.0.";
                remoteIp += std::to_string(j + 1);
                endpoints.emplace_back(*ioService, remoteIp, port, isHost?osuCrypto::SessionMode::Server:osuCrypto::SessionMode::Client, name);
            }
            channels[j] = new osuCrypto::Channel(endpoints.back().addChannel("chl", "chl"));
        }
    }
    ...
    if (isClient) {
        ...
        for (uint32_t j = 0; j < tileNum; ++j) {
            if (j != tileIndex) {
                PublicKeyEntry* pke = new PublicKeyEntry();
                ...
                // #ifdef USE_FHE
                sendFHEPublicKey(j, cryptoUtil.getFHEPublicKey());
                // #endif
            }
        }
        ...
        for (uint32_t j = 0; j < tileNum; ++j) {
            if (j != tileIndex) {
                recvEntrance(j); // Receive Paillier pub key

                // #ifdef USE_FHE
                recvEntrance(j); // Receive FHE pub key
                // #endif
            }
        }        
    ...
    for (uint32_t i = 0; i < tileNum; ++i) {
        if (i != tileIndex) {
            threads.emplace_back([this, i] () {
                close_mpc_channel(this->isClient_, i);
            });
        }
    }
    for (auto& thrd : threads) thrd.join();
...
void TaskComm::sendTaskDigest(Task task, const uint32_t dstTid) {
    ...
    channels[dstTid]->asyncSend(std::move(serial_str));
...
void TaskComm::sendDecrypt(CipherEntry& ce, const uint32_t dstTid, const uint32_t taskId, const uint32_t operandId0) {
    ...
    channels[ce.tid]->asyncSend(std::move(serial_str));   
...
void TaskComm::sendDecryptPacked(std::shared_ptr<PackedCipherEntry> pce) {
    ...
    channels[pce->tid]->asyncSend(std::move(serial_str));   
...
void TaskComm::decryptHandler(CipherEntry& ce, const uint32_t srcTid, const uint32_t dstTid, const uint32_t taskId, const uint32_t operandId0) {
    ...
    ShareVec shareVec;
    cryptoUtil.decryptToInt64Vec(ct, shareVec, ce.plainNum);
    // printf("decrypt share Vec %d, %lld, %lld\n", ce.plainNum, shareVec[0], shareVec[1]);
...
void TaskComm::decryptPackedHandler(const uint32_t srcTid) {
    ...
    for (int i=0; i<packed_ce_q[srcTid].size(); ++i) {
        ...
        for (int m=0; m<pce->taskId.size(); ++m) {
            ...
            ShareVec shareVec;
            for (int kk=0; kk<plainNumPerCE; ++kk) {
                shareVec.push_back(pce->share[cur_start + kk]);
            }
...
void TaskComm::writeShareHandler(const ShareVec& shareVec, const uint32_t srcTid, const uint32_t taskId, const uint32_t operandId0) {
    ...
    TaskPayload* tp = (TaskPayload*)task.buf;
    tp->isOperandEncrypted[operandId0] = true;
    for (int i=0; i<shareVec.size(); ++i)
        tp->writeShareToOperand(shareVec[i], operandId0, i);
    tp->setCipherEntryPlainNum(operandId0, shareVec.size());
...
int TaskComm::recvEntrance(const uint32_t srcTid) {
    ...
    channels[srcTid]->recv(serial_str);
    ...
    if (cmd_str == "TASK_DIGEST") {
        ...
        uint64_t cur_taskv_size = this->merge_encrypted_taskq[srcTid].size();
        ...
        delete pke;
    } else if (cmd_str == "FHE_PUBLIC_KEY") {
        string buf;
        size_t pk_size;
        size_t gk_size;
        size_t autok_size;
        ia >> buf >> pk_size >> gk_size >> autok_size;
        cryptoUtil.setUpRemoteFHECipher(srcTid, buf, pk_size, gk_size, autok_size);
...
void TaskComm::computeTaskq(vector<Task>& taskq, uint32_t curTid, uint32_t dstTid, int party, int rotation) {
    ...
    uint64_t cur_taskv_size = taskq.size() - rotation;
    ...
    uint64_t step = (cur_taskv_size / compute_thread_num) + 1;
...
void TaskComm::ssAddPairDoubleUint(Task& task, int party) {
	...
	for (int i=0; i<2; ++i) {
		if (!task_content->isOperandEncrypted[i]) {
            ...
            std::vector<uint64_t> shareVec = {task_content->operands[i].enc.share[0], task_content->operands[i].enc.share[1]}; 
	...
	// Generate new shares
    uint64_t new_share_a0 = cryptoUtil.getRandomShare();
    emp::Integer share_a0 = cryptoUtil.shareToEmpInteger(new_share_a0, emp::ALICE);
    emp::Integer share_a1 = operands[0].a - share_a0;
    uint64_t new_share_a1 = cryptoUtil.empIntegerToShare(share_a1, emp::BOB);

    uint64_t new_share_b0 = cryptoUtil.getRandomShare();
    emp::Integer share_b0 = cryptoUtil.shareToEmpInteger(new_share_b0, emp::ALICE);
    emp::Integer share_b1 = operands[0].b - share_b0;
    uint64_t new_share_b1 = cryptoUtil.empIntegerToShare(share_b1, emp::BOB);
...
void TaskComm::ssAddMixedPairDoubleUint(Task& task, int party) {
    ...
    uint64_t new_share_a1 = cryptoUtil.empFloatToShare(share_a1, emp::BOB);
...
void TaskComm::ssMinUintWithParent(Task& task, int party) {
    ...
    // uint64_t a1 = cryptoUtil.empIntegerToShare(operands[0].val, emp::PUBLIC);
    // uint64_t a2 = cryptoUtil.empIntegerToShare(operands[1].val, emp::PUBLIC);
    // uint64_t a3 = cryptoUtil.empIntegerToShare(operands[0].parent, emp::PUBLIC);
    // uint64_t a4 = cryptoUtil.empIntegerToShare(operands[1].parent, emp::PUBLIC);
	...
	// Generate new shares
    uint64_t new_share_val0 = cryptoUtil.getRandomShare();
    uint64_t new_share_parent0 = cryptoUtil.getRandomShare();
    ...
    uint64_t new_share_val1 = cryptoUtil.empIntegerToShare(share_val1, emp::BOB);    
    uint64_t new_share_parent1 = cryptoUtil.empIntegerToShare(share_parent1, emp::BOB);
...
void TaskComm::ssAddDouble(Task& task, int party) {
	...
	// Generate new shares
    uint64_t new_share_0 = cryptoUtil.getRandomShare();
    emp::Float share_0 = cryptoUtil.shareToEmpFloat(new_share_0, emp::ALICE);
    emp::Float share_1 = operands[0] - share_0;
    uint64_t new_share_1 = cryptoUtil.empFloatToShare(share_1, emp::BOB);
...
void TaskComm::ssAddUlong(Task& task, int party) {
    ...
    uint64_t new_share_1 = cryptoUtil.empIntegerToShare(share_1, emp::BOB);
...
void TaskComm::ssSwapCipherEntry(Task& task, int party) {
	...
	// Generate new shares
    uint64_t new_share_val00 = cryptoUtil.getRandomShare();
    uint64_t new_share_val10 = cryptoUtil.getRandomShare();
    ...
    uint64_t new_share_val01 = cryptoUtil.empIntegerToShare(share_val01, emp::BOB);    
    uint64_t new_share_val11 = cryptoUtil.empIntegerToShare(share_val11, emp::BOB);
...
void TaskComm::sendFHEPublicKey(const uint32_t dstTid, troyn::FHEWrapper& fhe) {
    std::string cmd_str = "FHE_PUBLIC_KEY";

    std::string buf;
    size_t pk_size;
    size_t gk_size;
    size_t autok_size;
    buf = fhe.serialize_pub_key(pk_size, gk_size, autok_size);
    ...
    oa << cmd_str << buf << pk_size << gk_size << autok_size;
...
void TaskComm::sendFinish() {
    ...
    for (int dstTid=0; dstTid<tileNum_; dstTid++) {
        if (dstTid != tileIndex_) {
            channels_mtx[dstTid].lock();
            if (!channels[dstTid]) printf("Unexpected null channel\n");
            channels[dstTid]->asyncSendCopy(cmd_str); 
            channels_mtx[dstTid].unlock();  
        }
    } 
}
...
void TaskComm::doTaskOperandRotation(std::vector<Task>& taskv, uint64_t rotation) {
    if (rotation != 0) {
        uint64_t end = (uint64_t)(taskv.size() - rotation);
        for (int j=0; j<end; ++j) {
...
void TaskComm::sendCipherEntryVec(const CipherEntryVec& cev, const uint32_t dstTid) {
    ...
    auto sendCEV = [this, &curChannels, dstTid](const CipherEntryVec& cur_cev) {
        // printf("send cur_cev.ceVecs[0].plainNum %d\n", cur_cev.ceVecs[0].plainNum);
        std::string serial_str;
        boost::iostreams::back_insert_device<std::string> inserter(serial_str);
        boost::iostreams::stream<boost::iostreams::back_insert_device<std::string>> s(inserter);
        boost::archive::binary_oarchive oa(s); 
        ...
        s.flush();
        curChannels[dstTid]->asyncSend(std::move(serial_str));        
    };
    ...
    uint64_t step = MAX_SEND_CEV_SIZE;
    uint64_t vecSize = cev.ceVecs.size();
    for (uint64_t i=0;i<vecSize;i+=step) {
        uint64_t endIndex = i+step<vecSize? i+step:vecSize;
...
void TaskComm::sendCiphertextVec(const std::vector<std::string>& cev, const uint32_t dstTid) {
    channels_mtx[dstTid].lock();

    std::vector<osuCrypto::Channel*> curChannels(channels);
    auto sendCEV = [this, &curChannels, dstTid](const std::vector<std::string>& cur_cev) {
        std::string packed_cur_cev;
        std::vector<size_t> packed_size_cur_cev;
        packed_cur_cev = sci::pack_string_vec(cur_cev, packed_size_cur_cev);
        std::string send_cmd_str = "CIPHERTEXT_VEC";
        curChannels[dstTid]->asyncSend(std::move(send_cmd_str));
        curChannels[dstTid]->asyncSend(std::move(packed_cur_cev));     
        curChannels[dstTid]->asyncSend(std::move(packed_size_cur_cev));   
    };

    auto sendCMD = [this, &curChannels, dstTid](const std::string cur_cmd) {
        curChannels[dstTid]->asyncSend(std::move(cur_cmd));        
    };

    std::string begin_cmd_str = "BEGIN_CIPHERTEXT_VEC";
    std::string end_cmd_str = "END_CIPHERTEXT_VEC";

    sendCMD(begin_cmd_str);

    uint64_t step = MAX_SEND_CEV_SIZE;
    uint64_t vecSize = cev.size();
    for (uint64_t i=0;i<vecSize;i+=step) {
        uint64_t endIndex = i+step<vecSize? i+step:vecSize;
        auto first = cev.cbegin() + i;
        auto last = cev.cbegin() + endIndex;
        std::vector<std::string> curCEV(first, last);
        sendCEV(curCEV);
    }
...
void TaskComm::recvCiphertextVec(std::vector<std::string>& cev, const uint32_t srcTid) {
    std::string begin_cmd_str = "BEGIN_CIPHERTEXT_VEC";
    std::string end_cmd_str = "END_CIPHERTEXT_VEC";
    std::string send_cmd_str = "CIPHERTEXT_VEC";
    cev.clear();
    while (true) {
        std::string cmd_str;
        channels[srcTid]->recv(cmd_str);
        std::vector<std::string> curCEV;
        std::string packed_curCEV;
        std::vector<size_t> packed_size_curCEV;
        ...
        if (cmd_str == begin_cmd_str) {
            ...
            channels[srcTid]->recv(packed_curCEV);
            channels[srcTid]->recv(packed_size_curCEV);
            curCEV = sci::unpack_string_vec(packed_curCEV, packed_size_curCEV);
            cev.insert(cev.end(), curCEV.begin(), curCEV.end());
            continue;
        } else {
            printf("Did not receive expected CIPHERTEXT_VEC!\n");
...
void TaskComm::recvCipherEntryVec(CipherEntryVec& cev, const uint32_t srcTid) {
    ...
    while (true) {
        ...
        if (cmd_str == begin_cmd_str) {
            ...
            ia >> curCEV;
            // printf("recv curCEV.ceVecs[0].plainNum %d\n", curCEV.ceVecs[0].plainNum);
...
void TaskComm::sendShareVec(const ShareVec& sv, const uint32_t dstTid) {
    channels_mtx[dstTid].lock();
    channels[dstTid]->asyncSend(std::move(sv)); 
    channels_mtx[dstTid].unlock(); 
}

void TaskComm::recvShareVec(ShareVec& sv, const uint32_t srcTid) {
    channels[srcTid]->recv(sv);
}

void TaskComm::sendShareTensorVec(const ShareTensorVec& stv, const uint32_t dstTid) {
    std::string cmd_str = "SHARE_TENSOR";
...
void TaskComm::recvShareTensorVec(ShareTensorVec& stv, const uint32_t srcTid) {
    std::string serial_str;
    channels[srcTid]->recv(serial_str);
    ...
    if (cmd_str != "SHARE_TENSOR") {
        printf("Did not receive expected SHARE_TENSOR!\n");
        exit(-1);
    }
}
