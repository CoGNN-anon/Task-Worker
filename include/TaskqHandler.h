...
public:
    ...
    void isNoDummyEdgeIs(bool isNoDummyEdge) {
        isNoDummyEdge_ = isNoDummyEdge;
    }

    bool getIsNoDummyEdge() {
        return isNoDummyEdge_;
    }
    ...
    Semaphore& getRemoteWeightReadySmp() {
        return remote_weight_ready_smp;
    }

    Semaphore& getWeightAvgFinishedSmp() {
        return weight_avg_finished_smp;
    }
    ...
    osuCrypto::Channel* getChannel(const uint32_t dstTid) {
        return channels[dstTid];
    }

    osuCrypto::SilentOtExtReceiver* getOTRecverPtr(const uint32_t dstTid) {
        return ot_recvers[dstTid];
    }

    osuCrypto::SilentOtExtSender* getOTSenderPtr(const uint32_t dstTid) {
        return ot_senders[dstTid];
    }
    ...
    void sendCiphertextVec(const std::vector<std::string>& cev, const uint32_t dstTid);
    void recvCiphertextVec(std::vector<std::string>& cev, const uint32_t srcTid);
    ...
    void sendShareVec(const ShareVec& sv, const uint32_t dstTid);
    void recvShareVec(ShareVec& sv, const uint32_t srcTid);

    void sendShareTensorVec(const ShareTensorVec& stv, const uint32_t dstTid);
    void recvShareTensorVec(ShareTensorVec& stv, const uint32_t srcTid);
    ...
    void sendFHEPublicKey(const uint32_t dstTid, troyn::FHEWrapper& fhe);
    void sendFinish();
    void recvFinish();
...
private:
    ...
    Semaphore remote_weight_ready_smp;
    Semaphore weight_avg_finished_smp;
    uint32_t basePort;
    std::string ip;
    uint32_t mpcBasePort;
    std::string mpcIp;
    std::list<osuCrypto::Session> endpoints;
    osuCrypto::IOService* ioService;
    size_t tileNum_;
    size_t tileIndex_;
    ...
    bool isCluster_;
    bool isNoDummyEdge_;
    std::vector<osuCrypto::SilentOtExtReceiver*> ot_recvers;
    std::vector<osuCrypto::SilentOtExtSender*> ot_senders;
